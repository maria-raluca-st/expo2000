...
@Component({
  selector: 'current-expos-page',
  templateUrl: './current-expos-page.component.html',
  styleUrls: ['./current-expos-page.component.scss']
})
export class CurrentExposPageComponent implements OnInit {
  ...
  noTodayExposArt!: number;
  noTodayExposPhoto!: number;
  noTodayExposOther!: number;
  ...
  //Function to get expos and account details
  async getStarted() {
    this.noTodayExposArt = 0;
    this.noTodayExposPhoto = 0;
    this.noTodayExposOther = 0;
    var Expos: expo[];
    Expos = [];
    ...
    for (let i = 0; i < this.allExpos.length; i++) {
      if (this.allExpos[i].expiration_date === this.todayDate && !this.allExpos[i].completed) {
        switch (this.allExpos[i].label) {
          case "art":
            this.noTodayExposArt = this.noTodayExposArt + 1;
            break;
          case "photo":
            this.noTodayExposPhoto = this.noTodayExposPhoto + 1;
            break;
          case "other":
            this.noTodayExposOther = this.noTodayExposOther + 1;
            break;
        }
      }
    }
  ...
  //Function to fetch the expos from database
  getExpos() {
